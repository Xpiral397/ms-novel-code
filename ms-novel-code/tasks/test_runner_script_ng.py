...
def create_environment_directories():
    ...
    for dir_path in env_dirs:
        try:
            ...
            # Don't fail if we can't create these dirs, just continue
            print(f"Warning: Could not create environment directory {dir_path}: {e}", file=sys.stderr)
...
class CustomTestResult(unittest.TestResult):
    def __init__(self):
        ...
        self.subtest_failures = []  # Track subtest failures
    ...
    def addSubTest(self, test, subtest, err):
        """Handle subtest results - called for each subTest() within a test method."""
        super().addSubTest(test, subtest, err)

        if err is not None:
            # Subtest failed or had error
            self.subtest_failures.append({
                "subtest": str(subtest),
                "exception": f"{err[0].__name__}: {str(err[1])}" if err else None,
                "traceback": "".join(traceback.format_exception(*err)) if err else None,
            })
    ...
    def stopTest(self, test):
        ...
        if hasattr(self, 'subtest_failures') and self.current_test_output:
            # Check if this test was already added via addSuccess/addFailure/addError
            test_already_added = any(
                detail["test_name"] == self.current_test_output["test_name"]
                for detail in self.test_results["details"]
            )

            if not test_already_added:
                # This test had only subtests, no main test failure/error
                if self.subtest_failures:
                    # Some subtests failed
                    self.test_results["summary"]["failed"] += 1
                    self.current_test_output["passed"] = False

                    # Combine subtest failure information
                    failure_info = []
                    for failure in self.subtest_failures:
                        failure_info.append(f"Subtest {failure['subtest']}: {failure['exception']}")

                    self.current_test_output["exception"] = "; ".join(failure_info)
                    # Use the first subtest's traceback as representative
                    if self.subtest_failures[0]["traceback"]:
                        self.current_test_output["traceback"] = self.subtest_failures[0]["traceback"]
                else:
                    # All subtests passed
                    self.test_results["summary"]["passed"] += 1
...
def run_tests(task_dir: Path):
    ...
    try:
        ...
        def patched_run(self, test):
            # Use the standard unittest execution flow to ensure setUpClass/tearDownClass are called
            # We need to capture output for individual test cases, so we'll patch the test case run method
            original_test_run = unittest.TestCase.run

            def patched_test_run(test_case, result):
                # Capture output for individual test cases
                with OutputCapturer(test_result, test_case):
                    # Use the original test case run method which handles setUp/tearDown
                    return original_test_run(test_case, result)

            # Temporarily patch the test case run method
            unittest.TestCase.run = patched_test_run

            try:
                # Use the standard suite execution which will call setUpClass/tearDownClass
                # for test classes and handle the proper test execution order
                # But use our custom test result instead of the default one
                return test.run(test_result)
            finally:
                # Restore the original test case run method
                unittest.TestCase.run = original_test_run
...
if __name__ == "__main__":
    ...
    if not is_in_virtual_environment():
        ...
        # --- Change CWD --- #
    # Handle working directory robustly - attempt to get current dir but don't fail if it's inaccessible
    original_cwd = None
    try:
        original_cwd = os.getcwd()
    except (OSError, FileNotFoundError):
        # Current directory might be inaccessible/deleted - this is fine, we'll just set it explicitly
        pass
    ...
    if original_cwd is not None:
        os.chdir(original_cwd)  # Only restore if we saved it
